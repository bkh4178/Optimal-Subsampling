...
# 7. A_hat_pi^{-1} 근사 오차 (Ainv_frob_error) — C1 vs. ER 불일치 원인 진단
# 이론: oracle이 C1을 최소화하지만 finite sample에서 A_hat_pi^{-1} ≈ A^{-1} 근사가
#       concentration이 심할수록 불안정해져 ER이 나빠질 수 있음
cat("\n\n========== 7. A_hat_pi^{-1} 근사 오차 (mean Ainv_frob_error) ==========\n")
cat("  이론: oracle이 C1 1위지만 Ainv_frob_error도 가장 크면 → remainder 문제 확인\n\n")

cat(sprintf("  %-40s %6s %6s %14s %14s %10s\n",
            "method", "k", "gamma", "Ainv_frob_err", "A_pi_cond", "solve_fail%"))
...
for (gv in gamma_vals) {
  ...
  for (kv in c(500, 1000)) {
    cat(sprintf("  k = %d\n", kv))
    for (m in key_methods) {
      sub <- sim_all[sim_all$method == m &
                     sim_all$gamma == gv &
                     sim_all$k == kv, ]
      if (nrow(sub) == 0) next
      frob  <- mean(sub$Ainv_frob_error, na.rm = TRUE)
      cond  <- mean(sub$A_pi_cond, na.rm = TRUE)
      fails <- mean(sub$A_solve_fail, na.rm = TRUE) * 100
      cat(sprintf("    %-40s %14.4f %14.2f %10.2f\n", m, frob, cond, fails))
    }
    cat("\n")
  }
}

# Ainv_frob_error rank vs. ER rank 비교 (k=500)
cat("\n========== 7b. Ainv_frob_error rank vs. ER rank (k=500) ==========\n")
cat("  Ainv_frob_error rank와 ER rank가 일치하면 → A^{-1} 근사 오차가 ER 결정\n")
cat(sprintf("  %-40s %6s %12s %8s\n", "method", "gamma", "Ainv_rank", "ER_rank"))
for (gv in gamma_vals) {
  frob_vals <- sapply(key_methods, function(m) {
    v <- sim_all$Ainv_frob_error[sim_all$method == m &
                                  sim_all$gamma == gv &
                                  sim_all$k == 500]
    if (length(v) == 0) NA else mean(v, na.rm = TRUE)
  })
  er_vals <- sapply(key_methods, function(m) {
    v <- sim_all$excess_risk[sim_all$method == m &
                             sim_all$gamma == gv &
  ...
  frob_rank <- rank(frob_vals, na.last = TRUE)
  er_rank   <- rank(er_vals, na.last = TRUE)
  for (i in seq_along(key_methods)) {
    cat(sprintf("  %-40s %6.1f %12.0f %8.0f\n",
                key_methods[i], gv, frob_rank[i], er_rank[i]))
  }
  cat("\n")
}