...
# 변화: n0 in n0 grid
# Plugin 3종: correct_scale, mild_omitted, severe_omitted (모두 plugin-param 계열,
#   nonparam은 함수형태 가정이 없어 "오명세"라는 개념이 적용되지 않으므로 제외)
# 베이스라인 5종: FULL, SRS, LEV-IPW, OPT-homo, OPT-hetero-oracle
#
# metric 체계는 exp4(05_exp4_misspecification_optional.R)와 완전히 동일:
...
run_one_rep_exp5 <- function(dat, k, X_test, mu_test, y_test, mse0, n0) {
  ...
  run_method <- function(method_name, misspec_setting, score,
    ...
    # Metric 1: leading term (메인 파이프라인/exp4와 동일 수식)
...
cat("====================================================\n")
cat(" Exp5: Misspecification x n0 Sensitivity\n")
cat("====================================================\n")
