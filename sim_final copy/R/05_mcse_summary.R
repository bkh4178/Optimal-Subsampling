...
paired_diff_summary <- function(df, m1, m2, metric = "excess_risk") {
  cols <- c("rep", "k", metric)
...
print_paired_diffs <- function(df, ks, metric = "excess_risk",
                               pairs = list(
                                 c("SRS",               "OPT-hetero-plugin"),
                                 c("OPT-homo",          "OPT-hetero-plugin"),
                                 c("OPT-hetero-plugin", "OPT-hetero-oracle")
                               )) {
  cat(sprintf("\n%s\n", strrep("=", 78)))
