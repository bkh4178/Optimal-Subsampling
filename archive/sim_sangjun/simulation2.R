...
x2 <- rnorm(N, 1, 2)                  # 연속형 (정규분포)
x3 <- rnorm(N, 1, 1)                  # 연속형 (정규분포)
x4 <- rbinom(N, 1, 0.7)               # 범주형 (매우 흔함 p=0.7)
...
N_test <- 10000
x1_t <- rnorm(N_test, 0, 1); x2_t <- rnorm(N_test, 1, 2)
x3_t <- rnorm(N_test, 1, 1); x4_t <- rbinom(N_test, 1, 0.7)
...
x1 <- rnorm(N, 0, 1); x2 <- rnorm(N, 1, 2)
x3 <- rnorm(N, 1, 1); x4 <- rbinom(N, 1, 0.7)
