...
run_opt_hetero_shrink <- function(dat, k, lambda = 0.5) {
  N <- nrow(dat$X)
  
  pi_oracle  <- compute_pi(score = sqrt(dat$sigma2 * dat$ell), k = k)
  pi_uniform <- rep(k / N, N)
  
  pi_shrink <- compute_pi(
    score = lambda * pi_oracle + (1 - lambda) * pi_uniform,
    k = k
  )
  ...
  list(
    method     = paste0("OPT-hetero-shrink-", lambda),
