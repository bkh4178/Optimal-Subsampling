# run_sim_lowdim7_homo.R
# 저차원 시뮬레이션 (p=2, intercept 포함) — v7
#
# v6 대비 변경사항:
...
estimate_sigma2_abslinear <- function(dat, idx_pilot, beta_pilot) {
  ...
  # E[resid^2] = sigma^2 이므로 resid^2 ~ abs(x1+x2)로 선형 적합
...
run_one_rep <- function(dat, k, X_test, mu_test, y_test) {
  ...
  results[["OPT-hetero-oracle-2s"]] <- run_method("OPT-hetero-oracle-2s", sqrt(dat$sigma2_vec * dat$ell))
...
cat("====================================================\n")
cat(" 저차원 시뮬레이션 v7 homo (p=2, 등분산)\n")
cat(sprintf(" N=%d / N_test=%d / n0=%d / n_rep=%d\n", N, N_test, n0, n_rep))
cat(sprintf(" k: %s\n", paste(ks, collapse = ", ")))
cat(" sigma(x) = 1 (등분산)\n")
cat(" variance model: sigma = gamma_0 + |gamma_1*x1 + gamma_2*x2| (등분산에서 misspecification)\n")
cat(" sampling: n0 then k (pilot 모집단 되돌림, 전체 N에서 k개 sampling)\n")
cat("====================================================\n")

dat      <- generate_data(N, beta, seed = 456)
...
saveRDS(results_df, "sim/results_lowdim7_homo.rds")
cat(sprintf("\n저장 완료: sim/results_lowdim7_homo.rds (%d행)\n", nrow(results_df)))
...
cat(strrep("-", 65), "\n")
plugin_diag <- results_df[!results_df$fail &
                           results_df$method == "OPT-hetero-plugin", ]

if (nrow(plugin_diag) > 0 && any(!is.na(plugin_diag$diag_sigma_cor))) {
  agg_diag <- aggregate(cbind(diag_sigma_cor, diag_score_cor) ~ method + k,
                        data = plugin_diag, FUN = mean, na.rm = TRUE)
  for (k_ in ks) {
...
for (k_ in ks) {
  ...
  cat(sprintf("  plugin / homo:          %.4f → %s\n",
              plugin_ex / homo_ex,
              ifelse(plugin_ex < homo_ex, "plugin wins ✓", "plugin loses to homo")))
}
