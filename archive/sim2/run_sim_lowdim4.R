...
#   - k = 500, 1000, 2000
#   - Train data 고정 (루프 밖 1회 생성) — sampling randomness만 반영
#   - SRS/homo/oracle: pilot 없이 k개 온전히 사용
#   - plugin: pilot n0개 (탐색 비용) + second-stage k-n0개
#   - Clipping: iterative budget 재분배 방식 + clip 개수 출력
#
...
run_one_rep <- function(dat, k, X_test, mu_test, y_test) {
  ...
  # ── Method 실행 함수 ────────────────────────────────────────────────────────
  run_method <- function(method, score, use_pilot = FALSE) {

    if (!use_pilot) {
      # SRS / homo / oracle: k개 전체 Bernoulli sampling
      ...
      # plugin: pilot n0개 고정 포함 + second-stage k-n0개
      budget     <- k - n0
      pi_raw     <- compute_pi(score, budget)
      pi_full    <- pi_raw
      pi_full[idx_pilot] <- 0               # 중복 방지
      idx_second <- which(rbinom(N, 1, pi_full) == 1)
      idx_final  <- union(idx_pilot, idx_second)
      pi_ipw     <- pi_raw
      pi_ipw[idx_pilot] <- n0 / N           # pilot IPW 가중치
      n_clip     <- sum(pi_raw >= 1)
    }
  ...
  # SRS / homo / oracle: k개 온전히 사용
  results[["SRS"]]               <- run_method("SRS",               rep(1, N),                       use_pilot = FALSE)
  results[["OPT-homo"]]          <- run_method("OPT-homo",          sqrt(dat$ell),                   use_pilot = FALSE)
  results[["OPT-hetero-oracle"]] <- run_method("OPT-hetero-oracle", sqrt(dat$sigma2_vec * dat$ell),  use_pilot = FALSE)
  # plugin: pilot n0개 포함, second-stage k-n0개
  results[["OPT-hetero-plugin"]] <- run_method("OPT-hetero-plugin", sqrt(sigma2_hat * dat$ell),      use_pilot = TRUE)
...
cat(" SRS/homo/oracle: k개 / plugin: pilot n0 + second k-n0\n")
cat("====================================================\n")
