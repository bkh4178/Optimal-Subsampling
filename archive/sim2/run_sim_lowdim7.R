...
# - 변경: E[resid^2] = sigma^2(x)를 이용해 resid^2 ~ |x1+x2| 선형 모델로 sigma^2 추정
#   → sigma2_hat = predict(var_model), score = sqrt(sigma2_hat * ell)
...
cat("====================================================\n")
cat(" 저차원 시뮬레이션 v7 (p=2, intercept 포함)\n")
cat(sprintf(" N=%d / N_test=%d / n0=%d / n_rep=%d\n", N, N_test, n0, n_rep))
cat(sprintf(" k: %s\n", paste(ks, collapse = ", ")))
cat(" sigma(x) = |x1+x2| + 0.5\n")
cat(" variance model: sigma^2(x) ~ |x1+x2| (resid^2 기반 선형 추정)\n")
...
saveRDS(results_df, "sim/results_lowdim7.rds")
cat(sprintf("\n저장 완료: sim/results_lowdim7.rds (%d행)\n", nrow(results_df)))
