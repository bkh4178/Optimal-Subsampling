# run_sim_p9_4_4.R
# Step 4-4: p=9 Mixed Covariate + Representative Mixed Variance + Grouped Beta
#
# Purpose:
#   Use the most mixed variance direction from Step 4-3 as a representative
#   realistic DGP, with grouped weak/middle/strong beta coefficients.
#
# DGP:
#   X = cbind(1, x1, ..., x9)
#   x1,x2,x3 ~ N(0,1)
#   x4,x5,x6 ~ Bernoulli(0.5)-0.5
#   x7,x8,x9 ~ Uniform(-sqrt(3), sqrt(3))
#   beta = c(1, 0.5,1.0,1.5, 0.5,1.0,1.5, 0.5,1.0,1.5)
#   sigma(x) = |x1 + x4 + x7| + 0.5
#
...
cat("====================================================\n")
cat(" p=9 Mixed Covariate + Representative Mixed Variance + Grouped Beta simulation (Step 4-4)\n")
cat(sprintf(" N=%d / N_test=%d / n0=%d / n_rep=%d\n", N, N_test, n0, n_rep))
cat(sprintf(" k: %s\n", paste(ks, collapse = ", ")))
cat(" X = cbind(1, x1, ..., x9)\n")
cat(" x1,x2,x3 ~ N(0,1)\n")
cat(" x4,x5,x6 ~ Bernoulli(0.5)-0.5\n")
cat(" x7,x8,x9 ~ Uniform(-sqrt(3), sqrt(3))\n")
cat(" beta grouped: intercept=1, each distribution has weak/middle/strong coefficients 0.5/1.0/1.5\n")
cat(" variance case: x1_x4_x7\n")
cat(" sigma(x)=|x1+x4+x7|+0.5\n")
cat(" variance model: resid^2 ~ |x1+x4+x7|; score = sqrt(sigma2_hat * ell)\n")
...
saveRDS(results_df, "sim/results_p9_mixed_x1x4x7_groupbeta.rds")
cat(sprintf("\n저장 완료: sim/results_p9_mixed_x1x4x7_groupbeta.rds (%d행)\n", nrow(results_df)))
