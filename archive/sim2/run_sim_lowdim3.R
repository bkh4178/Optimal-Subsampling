...
run_one_rep <- function(dat, k, X_test, mu_test, y_test) {
  ...
  pilot_df$V2sq <- pilot_df$V2^2   # x1^2
  pilot_df$V3sq <- pilot_df$V3^2   # x2^2
  pilot_df$V2V3 <- pilot_df$V2 * pilot_df$V3  # x1*x2
  var_model   <- lm(log_resid2 ~ . - 1, data = pilot_df)
  full_df     <- as.data.frame(dat$X)
  colnames(full_df) <- paste0("V", seq_len(p))
  full_df$V2sq <- full_df$V2^2
  full_df$V3sq <- full_df$V3^2
  full_df$V2V3 <- full_df$V2 * full_df$V3
  ...
  for (method in names(scores)) {
    ...
    budget <- k - n0
    pi_raw <- compute_pi(scores[[method]], budget)

    # pilot 인덱스는 pi=0으로 고정 (이미 포함, 중복 방지)
    pi_full <- pi_raw
    pi_full[idx_pilot] <- 0
    ...
    # IPW용 pi: pilot은 n0/N, 나머지는 pi_raw
    pi_ipw <- pi_raw
...
cat(" [Train data 고정 — sampling randomness만 반영]\n")
cat("====================================================\n")

# train data 고정 (루프 밖 1회 생성)
