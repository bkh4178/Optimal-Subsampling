# run_sim_p3_mixed.R
# Step 3-2: p=3 Mixed Covariate simulation — v7
#
# Purpose:
#   Check whether the heteroscedasticity-aware sampling pattern remains stable
#   when one covariate distribution changes from Normal to centered Bernoulli.
#
# DGP:
#   X = cbind(1, x1, x2, x3)
#   x1, x2 ~ N(0,1)
#   x3 ~ Bernoulli(0.5) - 0.5
#   sigma(x) = |x1 + x2| + 0.5
#
# Plug-in variance model:
#   Fit resid^2 ~ |x1 + x2| on the pilot sample.
#   This estimates sigma^2(x), not sigma(x) directly.
#   The plug-in sampling score is sqrt(sigma2_hat * ell).
...
cat("====================================================\n")
cat(" p=3 Mixed Covariate simulation — v7\n")
...
saveRDS(results_df, "sim/results_p3_mixed.rds")
cat(sprintf("\n저장 완료: sim/results_p3_mixed.rds (%d행)\n", nrow(results_df)))
