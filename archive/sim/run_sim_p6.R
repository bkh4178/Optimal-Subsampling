# run_sim_p6.R
# Phase 1 시뮬레이션 실행 — p = 6
#
# 구조:
#   case in {1, 2, 3}  ×  k in {200, 500, 1000, 2000}  ×  500 replicates
#
# 공변량:
#   X1, X2 ~ N(0,1)              (weak signal: beta = 0.5)
#   X3, X4 ~ Bernoulli(0.5) centered  (moderate signal: beta = 1.0)
#   X5, X6 ~ Uniform(-sqrt(3), sqrt(3))  (strong signal: beta = 2.0)
#
# 저장: sim/results_p6.rds
...
N_test  <- 5000
p       <- 6
beta0   <- c(0.5, 0.5, 1.0, 1.0, 2.0, 2.0)
n0      <- 200
n_rep   <- 500
cases   <- c(1, 2, 3)
...
saveRDS(results_df, "sim/results_p6.rds")

elapsed_total <- round((proc.time() - time_start)["elapsed"], 1)

cat("\n====================================================\n")
cat(sprintf(" 저장 완료: sim/results_p6.rds\n"))
cat(sprintf(" 총 행 수:  %d\n", nrow(results_df)))
