# run_sim_p4.R
# Phase 1 시뮬레이션 실행 — p = 4
#
# 구조:
#   case in {1, 2}  ×  k in {200, 500, 1000, 2000}  ×  gamma in {1, 2, 4}  ×  500 replicates
#
# Case 2 sigma2: 1 + gamma * (X1^2 + X2^2)  — chi-squared(2) 기반 이분산
#
# 저장: sim/results_p4.rds
...
N_test  <- 5000
p       <- 4
beta0   <- c(0.5, 0.5, 1.0, 1.0)
n0      <- 200
n_rep   <- 500
cases   <- c(1, 2)
ks      <- c(200, 500, 1000, 2000)
gammas  <- c(1, 2, 4)   # Case 2: sigma2 = 1 + gamma * (X1^2 + X2^2)
...
saveRDS(results_df, "sim/results_p4.rds")

elapsed_total <- round((proc.time() - time_start)["elapsed"], 1)

cat("\n====================================================\n")
cat(sprintf(" 저장 완료: sim/results_p4.rds\n"))
cat(sprintf(" 총 행 수:  %d\n", nrow(results_df)))
cat(sprintf(" 총 소요:   %.1f초\n", elapsed_total))
cat("====================================================\n")

# ── 요약 테이블 (gamma × case × k × method 별 평균 excess_risk + 순위) ────────

cat("\n[요약] 평균 excess_risk — gamma × case × k × method  (괄호: 순위, FULL 제외)\n")

method_order <- c("FULL", "UNI-IPW", "LEV-IPW", "OPT-homo",
                  "OPT-hetero-oracle", "OPT-hetero-plugin")
rank_methods <- setdiff(method_order, "FULL")

agg <- aggregate(excess_risk ~ gamma + case + k + method, data = results_df, FUN = mean)
...
for (gm in gammas) {
  ...
  for (cs in cases) {
    ...
    rank_table <- list()
    for (k in ks) {
      sub <- agg[agg$gamma == gm & agg$case == cs & agg$k == k & agg$method %in% rank_methods, ]
    ...
    for (m in method_order) {
      cat(sprintf("%-22s", m))
      for (k in ks) {
        val <- agg$excess_risk[agg$gamma == gm & agg$case == cs & agg$k == k & agg$method == m]
        if (length(val) == 0) {
          cat(sprintf("%16s", "NA"))
        } else if (m == "FULL") {
...
cat("\n[SE 확인] gamma × case × k 별 excess_risk 표준오차\n")

agg_se <- aggregate(excess_risk ~ gamma + case + k + method, data = results_df,
...
for (gm in gammas) {
  ...
  for (cs in cases) {
    ...
    for (m in method_order) {
      cat(sprintf("%-22s", m))
      for (k in ks) {
        val <- agg_se$se[agg_se$gamma == gm & agg_se$case == cs & agg_se$k == k & agg_se$method == m]
